...
//    getOwnedNFTs?: () => Promise<PokemonNFT[]>;
    ...
    wonAuctions:Auction[]
    ownedNFTs: PokemonNFT[]; // <-- Add owned NFTs state
    isLoadingActiveAuctions: boolean; // <-- State provided by context
...
export const Web3Provider = ({ children }: { children: ReactNode }) => {
   ...
   const fetchActiveAuctions = useCallback(async () => {
    if (!auctionContract || !pokemonContract) { // Need both contracts
        console.log("Contracts not ready to fetch active auctions.");
        setActiveAuctions([]); // Clear state if contracts aren't ready
        return;
    }
    setIsLoadingActiveAuctions(true);
    try {
        console.log("Fetching active auctions from contract...");
        // Call the raw async function (defined outside), passing contracts
        ...
        setActiveAuctions(auctions); // Update state directly in the provider
    } catch (error) {
        console.error("Failed to fetch active auctions:", error);
        setActiveAuctions([]); // Clear state on error
    } finally {
        setIsLoadingActiveAuctions(false);
    }
}, [auctionContract, pokemonContract]); // Dependencies: Re-create if contracts change

    const fetchWonAuctions = useCallback(async () => {
    // Need auction contract, pokemon contract, and connected account
    if (!auctionContract || !pokemonContract || !account) {
        console.log("Contracts not ready or not connected to fetch won auctions.");
         setWonAuctions([]); // Clear state if prerequisites aren't met
        return;
    }
    setIsLoadingWonAuctions(true);
    try {
        console.log(`Workspaceing won auctions for ${account}...`);
        // Call the raw async function (defined outside), passing account and contracts
        ...
        setWonAuctions(auctions); // Update state directly in the provider
    } catch (error) {
        console.error(`Failed to fetch won auctions for ${account}:`, error);
        setWonAuctions([]); // Clear state on error
    } finally {
        setIsLoadingWonAuctions(false);
    }
}, [auctionContract, pokemonContract, account]); // Dependencies: Contracts and account

    const fetchOwnedNFTs = useCallback(async () => {
    if (!pokemonContract || !account) {
        console.log("Pokemon contract not ready or not connected to fetch owned NFTs.");
        setOwnedNFTs([]); // Clear state if prerequisites aren't met
        return;
    }
    setIsLoadingOwnedNFTs(true); // Start loading
    try {
        console.log(`Fetching owned NFTs for ${account}...`);
        const nfts = await getOwnedNFTs(account, pokemonContract); // Call the raw async function
        console.log("Fetched and setting owned NFTs state:", nfts);
        setOwnedNFTs(nfts); // Update state directly in the provider
    } catch (error) {
        console.error(`Failed to fetch owned NFTs for ${account}:`, error);
        setOwnedNFTs([]); // Clear state on error
    } finally {
        setIsLoadingOwnedNFTs(false); // End loading
    }
}, [account, pokemonContract]); // Dependencies: account and pokemonContract

   useEffect(() => {
       // We only want to set up listeners if the contract instance is available
       if (!auctionContract) {
         console.log("Auction contract not ready for event listeners.");
         return; // Exit if contract is null
       }
       if (!pokemonContract) {
       ...
       const handleAuctionCreated = (auctionId: bigint, seller: string, pokemonId: bigint, auctionEndTime: bigint, startingBid: bigint) => {
           ...
           fetchActiveAuctions(); // Call the useCallback function
           if(account)
            fetchOwnedNFTs();
       };
       ...
       const handleHighestBidIncreased = (bidder: string, amount: bigint, auctionId: bigint) => {
           ...
           fetchActiveAuctions(); // Call the useCallback function
           // If the bidder is the current user, or the previous highest bidder was the current user,
           // refreshing won auctions might be relevant (e.g., if they were outbid).
           // Only fetch if connected and account is available
           if (isConnected && account) {
                fetchWonAuctions(); // Call the useCallback function
           }
       };
        ...
        // Listener for NFTClaimed or NFTRedeemedBySeller
         const handleNFTFinalized = (auctionId: bigint, participant: string, pokemonId: bigint) => {
             console.log("Event: NFTClaimed or NFTRedeemedBySeller", { auctionId: Number(auctionId), participant, pokemonId: Number(pokemonId) });
              ...
              if (account && participant.toLowerCase() === account.toLowerCase()) {
                fetchOwnedNFTs(); // <<< Refresh owned NFTs if current user claimed/reclaimed
              }
              if (isConnected && account) {
                fetchWonAuctions(); // Call the useCallback function
              }
         ...
         const handlePokemonMint = (to: string, tokenId: bigint) => { // Adjust args based on your Mint event
            console.log("Event: MintedPokemon", { to, tokenId: Number(tokenId) });
            if (account && to.toLowerCase() === account.toLowerCase()) {
                fetchOwnedNFTs(); // <<< Refresh owned NFTs if current user received a mint
            }
        };
  
         // Listener for Withdrawal (Optional, mainly for logging)
         const handleWithdrawal = (user: string, amount: bigint, auctionId: bigint) => {
              ...
              // You might refetch won auctions here if pending returns are displayed in that list
              // if (isConnected && account) {
              //     getWonAuctions();
              // }
         };
       ...
       // --- Subscribe to Events ---
       // Use the contract instance to listen for specific events
       ...
       auctionContract.on("NFTClaimed", handleNFTFinalized); // Listen to NFTClaimed
       auctionContract.on("NFTRedeemedBySeller", handleNFTFinalized); // Listen to NFTRedeemedBySeller
       pokemonContract.on("MintedPokemon", handlePokemonMint); // Listen to NFTRedeemedBySeller

       // // auctionContract.on("Withdrawal", handleWithdrawal); // Optional listener
        
  
       // --- Cleanup Function ---
       // This function runs when the effect cleans up (component unmounts, or dependencies change)
       return () => {
         ...
         // auctionContract.off("Withdrawal", handleWithdrawal); // Clean up optional listener
          ...
          // Alternatively, you can remove *all* listeners for a specific event name:
          // auctionContract.removeAllListeners("AuctionCreated");
          // This is less safe if multiple parts of your app listen to the same event.
          // Removing the specific listener callback is generally preferred.
       };
  
     }, [auctionContract, pokemonContract, provider, isConnected, account, fetchActiveAuctions, fetchWonAuctions]); // Dependencies: Re-run if contract, fetch functions, connection or account change
...
useEffect(() => {
    ...
    fetchActiveAuctions(); // Fetch active auctions
